...
const sanitizeFileName = (value, fallbackName) => {
  const source = (value || fallbackName || "file").replace(/[?#].*$/, "").split("/").pop()?.trim();
...
const getSelectionValue = ({ selectedItem, label }) => {
  ...
  if (!remoteUrl) throw new Error(`The selected ${label} is missing a file URL.`);
  return remoteUrl;
};
...
const getSoundUploadValue = async (selectedSound) => {
  ...
  if (!response.ok) throw new Error("Unable to prepare the selected sound for upload. Please try another sound.");
  const blob = await response.blob();
  const blobExtension = blob.type?.split("/")?.[1]?.split(";")?.[0]?.trim() || "";
  const rawName = sanitizeFileName(selectedSound.fileName || selectedSound.title || remoteUrl, `calm-space-sound.${FALLBACK_SOUND_EXTENSION}`);
  const hasExtension = /\.[a-z0-9]+$/i.test(rawName);
  const fileName = hasExtension ? rawName : `${rawName}.${blobExtension || FALLBACK_SOUND_EXTENSION}`;
...
const getCalmSpaceVisuals = async (token) => {
  ...
  if (!token) throw new Error("Please sign in again to load calm-space visuals.");
  const response = await fetch(`${baseUrl}/api/media?page=1&limit=20`, {
    cache: "no-store",
    headers: { Authorization: `Bearer ${token}` },
  });
  const result = await response.json();
  if (!response.ok || !result?.success) throw new Error(result?.message || "Failed to fetch calm-space visuals.");
  return (result?.data?.media || [])
    .filter((item) =>
      item?.mediaType === "image" &&
      item?.status === "active" &&
      item?.url &&
      item?.categoryId?.categoryName?.trim()?.toLowerCase() === VISUAL_CATEGORY_NAME,
    )
    .map((item, index) => ({
      id: item?._id,
      image: item?.url,
...
const createCalmSpacePayload = async ({ description, selectedVisual, selectedSound }) => {
  const payload = new FormData();
  payload.append("describe", description.trim());
  payload.append("image", getSelectionValue({ selectedItem: selectedVisual, label: "image" }));
...
const MeditationSpaceApp = () => {
  ...
  useEffect(() => {
    uploadedVisualUrlRef.current = selectedVisual?.source === "upload" ? selectedVisual.image || "" : "";
  }, [selectedVisual]);

  useEffect(() => {
    uploadedSoundUrlRef.current = selectedSound?.source === "upload" ? selectedSound.url || "" : "";
  }, [selectedSound]);

  useEffect(() => {
    return () => {
      if (uploadedVisualUrlRef.current) URL.revokeObjectURL(uploadedVisualUrlRef.current);
    ...
    const fetchVisuals = async () => {
      try {
        ...
        if (activeJourneyId && token && baseUrl) {
          const access = await checkSessionAccess({ baseUrl, token, journeyId: activeJourneyId, requiredSession: 3 });
          if (!access.allowed && access.redirectTo) { router.replace(access.redirectTo); return; }
        }
        ...
        setVisualError(error?.message || "Unable to load calm-space visuals right now.");
        setVisuals([]);
  ...
  const handleSelectVisual = (visual) => {
    if (selectedVisual?.source === "upload" && selectedVisual?.image && selectedVisual.image !== visual?.image) {
  ...
  const handleUploadVisual = (file) => {
    ...
    if (selectedVisual?.source === "upload" && selectedVisual?.image) URL.revokeObjectURL(selectedVisual.image);
    setSelectedVisual({ id: `upload-image-${Date.now()}`, image: imageUrl, alt: file.name, fileName: file.name, file, source: "upload" });
    setSaveError("");
  };

  const handleSelectSound = (sound) => {
    if (selectedSound?.source === "upload" && selectedSound?.url && selectedSound.url !== sound?.url) {
  ...
  const handleUploadSound = (file) => {
    if (!file.type.startsWith("audio/")) { setSaveError("Please select a valid audio file."); return; }
    const soundUrl = URL.createObjectURL(file);
    if (selectedSound?.source === "upload" && selectedSound?.url) URL.revokeObjectURL(selectedSound.url);
    setSelectedSound({ id: `upload-sound-${Date.now()}`, title: file.name, fileName: file.name, url: soundUrl, file, mediaType: "audio", categoryName: "Uploaded", categorySlug: "uploaded", source: "upload" });
    setSaveError("");
  };

  const handleSave = async () => {
    const baseUrl = getBaseUrl();
    if (!baseUrl) { setSaveError("Calm space service is not configured."); return; }
    if (!token) { setSaveError("Please sign in again to save your calm space."); return; }
    if (!description.trim()) { setSaveError("Please describe your calm space before saving."); return; }
    try {
      setIsSaving(true);
      setSaveError("");
      const payload = await createCalmSpacePayload({ description, selectedVisual, selectedSound });
      ...
      const result = await response.json();
      if (!response.ok || !result?.success) throw new Error(result?.message || "Failed to save your calm space.");
      if (!result?.data?.soundLink) throw new Error("The sound file was not saved. Please try again or choose a smaller MP3/WAV file.");
      const activeJourneyId = localStorage.getItem("activeJourneyId");
      if (activeJourneyId && token && baseUrl) {
        await updateSessionProgress({ baseUrl, token, journeyId: activeJourneyId, compledSession: 3 });
      }
      router.push("/dashboard/EMDRCompanion");
    } catch (error) {
      console.error("Error saving calm space:", error);
      setSaveError(error?.message || "Unable to save your calm space right now.");
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="relative z-10 flex flex-col">
        <div className="text-center mb-6">
          ...
          <p className="text-stone-700 text-base lg:text-lg leading-relaxed max-w-2xl mb-2 mx-auto font-sans">
            Customize your personal sanctuary for moments of stress. Choose a visual that resonates with you.
          </p>
        </div>
